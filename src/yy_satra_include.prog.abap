...
CONSTANTS c_type_class TYPE trobjtype VALUE 'CLAS'.
CONSTANTS c_type_prog  TYPE trobjtype VALUE 'PROG'.
CONSTANTS c_type_fugr  TYPE trobjtype VALUE 'FUGR'.
CONSTANTS c_type_ldba  TYPE trobjtype VALUE 'LDBA'. " Log. DB
CONSTANTS c_type_tran  TYPE trobjtype VALUE 'TRAN'.
...
CLASS lcl_uml DEFINITION ABSTRACT FRIENDS lif_unit_test.
  PUBLIC SECTION.
    ...
    CONSTANTS c_uml_plant  TYPE tv_uml VALUE 0.
    CONSTANTS c_uml_graph  TYPE tv_uml VALUE 2.
    CONSTANTS c_uml_mscgen TYPE tv_uml VALUE 3.
    CONSTANTS c_uml_mock   TYPE tv_uml VALUE 9.
...
CLASS lcl_diagram_text DEFINITION ABSTRACT FRIENDS lif_unit_test.
  PUBLIC SECTION.
    CONSTANTS c_mode_aut TYPE char01 VALUE 'T'. " for ABAP Unit Test
    CONSTANTS c_mode_url TYPE char01 VALUE 'U'.
    CONSTANTS c_mode_txt TYPE char01 VALUE space.
    CONSTANTS c_mode_exe TYPE char01 VALUE 'E'.
...
CLASS lcl_diagram_plant_uml DEFINITION INHERITING FROM lcl_diagram_text FRIENDS lif_unit_test.
  ...
  PROTECTED SECTION.
    ...
    CONSTANTS c_charset_standard TYPE tv_base64
                                 VALUE 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/=' ##NO_TEXT.
    CONSTANTS c_charset_plantuml TYPE tv_base64
                                 VALUE '0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz-_0' ##NO_TEXT.
...
CLASS lcl_configuration DEFINITION CREATE PRIVATE FRIENDS lif_unit_test.
  PUBLIC SECTION.
    CLASS-METHODS get   RETURNING VALUE(rs_cfg) TYPE ts_diagram_config.
    CLASS-METHODS query RETURNING VALUE(rs_cfg) TYPE ts_diagram_config.
  ...
  PRIVATE SECTION.
    ...
    DATA mv_mode_exe TYPE flag.
    DATA mv_mode_txt TYPE flag.
...
CLASS lcl_class_name DEFINITION FRIENDS lif_unit_test.
  PUBLIC SECTION.
    METHODS is_global IMPORTING iv_name        TYPE sobj_name
                      RETURNING VALUE(rv_flag) TYPE xsdboolean.

    METHODS external CHANGING cv_clas TYPE program.

    METHODS technical IMPORTING iv_name         TYPE program
                      RETURNING VALUE(rv_class) TYPE program.

    METHODS to_internal IMPORTING iv_name         TYPE program
                        RETURNING VALUE(rv_class) TYPE program.
...
CLASS lcl_progress_indicator DEFINITION CREATE PRIVATE FRIENDS lif_unit_test.
  PUBLIC SECTION.
    ...
    CONSTANTS c_disable TYPE char01 VALUE '0'.
...
CONSTANTS c_id_func    TYPE satr_de_id VALUE 'U'. " Function module
CONSTANTS c_id_form    TYPE satr_de_id VALUE 'F'.
CONSTANTS c_id_class   TYPE satr_de_id VALUE 'R'. " Create object´/ Run time management
CONSTANTS c_id_method  TYPE satr_de_id VALUE 'm'.
CONSTANTS c_id_record  TYPE satr_de_id VALUE 'O'. " Record of measurement
CONSTANTS c_id_program TYPE satr_de_id VALUE 'P'. " Program
CONSTANTS c_id_skip    TYPE satr_de_id VALUE 'K'. " skip over SAP code
CONSTANTS c_id_load    TYPE satr_de_id VALUE 'L'. " Load
CONSTANTS c_id_module  TYPE satr_de_id VALUE 'M'. " Module call
CONSTANTS c_id_report  TYPE satr_de_id VALUE 'C'. " CALL screen/report
CONSTANTS c_id_dynpro  TYPE satr_de_id VALUE 'A'. " PBO / PAI
CONSTANTS c_id_flow    TYPE satr_de_id VALUE 'E'. " Flow Logic
" to be skipped
CONSTANTS c_id_title   TYPE satr_de_id VALUE 'Z'. " Dyn. Assign, Status/Title, SCAN, SET Local
CONSTANTS c_id_message TYPE satr_de_id VALUE 'W'. " Message/Continue/Wait
CONSTANTS c_id_db_oper TYPE satr_de_id VALUE 'V'. "  Database Operation

CONSTANTS c_id_sap_sql TYPE satr_de_id VALUE 'S'. " Open SQL
CONSTANTS c_id_db_sql  TYPE satr_de_id VALUE 'Q'. " Native SQL
CONSTANTS c_id_import  TYPE satr_de_id VALUE 'X'. " Export/Import
CONSTANTS c_id_db_tech TYPE satr_de_id VALUE 'N'. " Buffer/Technical DB Operation
...
CLASS lcl_filter IMPLEMENTATION.
  ...
  METHOD accepts.
    ...
    IF is_sat-id CA mv_config_id_reject.
      rv_flag = abap_false.
    ELSEIF ms_cfg-system_events = abap_false.
      rv_flag = xsdbool( is_sat-system <> c_system_flag ).
    ELSE.
      rv_flag = abap_true.
    ENDIF.
  ENDMETHOD.
...
CLASS lcl_filter_custom IMPLEMENTATION.
  ...
  METHOD constructor.
    ...
    mv_regex = concat_lines_of( table = ms_cfg-pattern
                                sep   = '|' ).
  ENDMETHOD.
  ...
  METHOD is_custom_code. " is object in customer name space ?
    TRY.
        rv_flag = xsdbool( mv_regex IS INITIAL OR 0 = numofchar( iv_name ) OR find_end( val   = iv_name
                                                                                        regex = mv_regex ) > 0 ).
...
*      If called object is local, its caller object must be custom code
    rv_flag = xsdbool( super->accepts( is_sat ) AND (
                                                      ( is_sat-called-global IS NOT INITIAL AND is_custom_code( is_sat-called-global ) )
                                                      OR ( is_sat-called-global IS INITIAL AND is_custom_code( is_sat-caller-global ) ) )     ).
  ENDMETHOD.
...
CLASS lcl_class_name IMPLEMENTATION.
  ...
  METHOD external.
    ...
    cv_clas = translate( val  = cv_clas(30)
                         from = '='
                         to   = ' ' ).
    cv_clas+30(2) = space.
  ENDMETHOD.
...
*    CLEAR rv_flag.
    SELECT SINGLE 'X'
      FROM tadir
      INTO @rv_flag            " Single Record buffered
      WHERE pgmid    = 'R3TR'
        AND object   = 'CLAS'
...
*   CALLED-Data
    ...
    IF NOT line_exists( mo_atra->it_tracetext[ mr_src->textindex ] ).
      RETURN.
    ENDIF.
    DATA(lv_tracetext) = CONV program( mo_atra->it_tracetext[ mr_src->textindex ]-tracetext ).  " type conversion

    IF module( line_index( mo_atra->it_traceprog[ cprog = mo_class_name->to_internal( lv_tracetext ) ] ) ) = 0.
      RETURN.
    ENDIF.
  ...
  METHOD set_constructor.
    ...
    IF line_exists( mo_atra->it_traceprog[ cprog = lv_tracetext ] ).
      RETURN.
    ENDIF.
...
*   Parse call hierarchy, apply custom filters, fill collection of messages
  ...
  METHOD put_caller.
    IF mi_collector->collect( is_sat ) = abap_false.
      skip_record( ).
    ENDIF.
...
CLASS lcl_actors IMPLEMENTATION.
  ...
  METHOD search.
    rv_idx = 0.
    IF is_object-type = c_type_class.
...
*       Inheritance cases: do not check object name
  ...
  METHOD new_object.
    rv_index = mv_last_number + 1.  " Next index - unique secondary key in actor collection
    mv_last_number = rv_index.
...
*   add a link to last valid entry to fill gaps created by filter logic
    rs_sat = is_sat.
    IF ms_previous IS NOT INITIAL AND ms_previous-called <> is_sat-caller AND is_sat-from_level > ms_previous-from_level.
...
CLASS lcl_uml IMPLEMENTATION.
  ...
  METHOD complete.
    " TODO: parameter IS_LIFELINE is never used (ABAP cleaner)
...
*   For performance: disable shadowing
    add_param( iv_cond    = xsdbool( is_cfg-shadowing = abap_false )
...
*   return and deactivate the actor
  ...
  METHOD participant.
    " TODO: parameter IV_CREATE is never used (ABAP cleaner)
  ...
  METHOD begin_loop.
    " TODO: parameter IV_FROM is never used (ABAP cleaner)
    " TODO: parameter IV_NAME is never used (ABAP cleaner)
  ...
  METHOD end_loop.
    " TODO: parameter IV_TO is never used (ABAP cleaner)
    " TODO: parameter IV_NAME is never used (ABAP cleaner)
...
*   Convert calls to UML sequence in text format
  ...
  METHOD message.
    " TODO: parameter IV_IDX is never used (ABAP cleaner)
...
*    mt_components = VALUE #( FOR t IN mt_trace ( t-message ) ).  dump on duplicates! does not pass test
    ...
    mt_xindex = VALUE #( FOR t IN mt_trace
                         ( line_index( mt_components[ table_line = t ] ) ) ).
  ENDMETHOD.


  METHOD to_trace.
    ct_trace = VALUE #( FOR x_idx IN mt_xindex
                        ( mt_components[ x_idx ] ) ).
  ENDMETHOD.
  ...
  METHOD compact.
    ...
    LOOP AT ct_xindex ASSIGNING FIELD-SYMBOL(<lv_x_index>).
      IF NOT mi_cycles->is_canonical_index( sy-tabix ).
        CONTINUE.
      ENDIF.
      APPEND <lv_x_index> TO lt_xindex.
    ENDLOOP.
    IF ct_xindex <> lt_xindex.
      rv_shrink = abap_true.
      ct_xindex = lt_xindex.
    ENDIF.
  ENDMETHOD.
...
*   duplicate entries are inserted BEFORE the existing row
  ...
  METHOD lif_cycles~is_canonical_index.
    LOOP AT mt_cycles TRANSPORTING NO FIELDS            "#EC CI_SORTSEQ
         WHERE end < iv_index AND last >= iv_index.
      rv_flag = abap_false.
      RETURN.
    ENDLOOP.
    rv_flag = abap_true.
...
*   rebuild (shrink) the loop reference table
    LOOP AT mt_cycles INTO DATA(ls_cycle).
      ...
      ENDAT.
      IF ls_cycle-last < ms_stop-start.
*       Loops entries before the active loop - verbatim
      ELSEIF ls_cycle-start < ms_stop-start AND ls_cycle-end >= ms_stop-last.
*       Loops entries starting before the active loop to be compacted and
*       that contains it (i.e. they enclose it ) - adjust
        ls_cycle-end  -= lv_delta.
        ls_cycle-last -= lv_delta.
      ELSEIF sy-tabix = mv_tabix.
*       Update the active loop entry
        ls_cycle = VALUE #( BASE ms_stop
                            last = ms_stop-end ).
      ELSEIF ls_cycle-start >= ms_stop-start AND ls_cycle-last <= ms_stop-end.
*       Loops entries completely enclosed by the active loop entry - verbatim
      ELSEIF ls_cycle-start > ms_stop-last.
*       Loops entries after the active loop - shifted by Delta
        ls_cycle-start -= lv_delta.
        ls_cycle-end   -= lv_delta.
        ls_cycle-last  -= lv_delta.
      ELSE.
        CONTINUE.
...
CLASS lcl_pattern IMPLEMENTATION.
  ...
  METHOD constructor.
    ...
    mv_size = COND #( WHEN iv_stop IS INITIAL
                      THEN lines( mt_xindex )
                      ELSE iv_stop ).
    skip( iv_start - 1 ).
  ENDMETHOD.
...
*   Pass the cycle information to interface LIF_CYCLES
    WHILE has_next( ).
      ...
      DO.                                               "#EC CI_NESTED.
        ...
        IF NOT ( lv_index IS INITIAL OR match_pattern( lv_index - 1 ) ).
          CONTINUE.
        ENDIF.
        EXIT.
      ENDDO.
...
CLASS lcl_file_name IMPLEMENTATION.
  ...
  METHOD dialog.
    ...
    cl_gui_frontend_services=>file_save_dialog( EXPORTING  window_title      = ms_file-title                " Window Title
                                                           default_extension = ms_file-ext             " Default Extension
                                                CHANGING   filename          = ms_file-name          " File Name to Save
...
CLASS lcl_file IMPLEMENTATION.
  ...
  METHOD create.
    rv_subrc = 1.
    IF io_name->dialog( ) = cl_gui_frontend_services=>action_cancel.
      RETURN.
    ENDIF.
...
*   for PlantUML Server: Convert to UTF-8, then deflate, then encode (base64 variant)
    cl_abap_gzip=>compress_binary( EXPORTING raw_in         = to_xstring( mv_diagram )  ##TYPE  " UTF-8
                                             compress_level = 9
                                   IMPORTING gzip_out       = DATA(lv_bin) ).
  ...
  METHOD local_plant_uml.
    ...
    IF lcl_file=>create( iv_data = to_xstring( mv_diagram )
                         io_name = lo_name ) IS NOT INITIAL.
      RETURN.
    ENDIF.
  ...
  METHOD registry_local.
    CLEAR rv_reg.
    cl_gui_frontend_services=>registry_get_value( EXPORTING  root      = cl_gui_frontend_services=>hkey_local_machine
                                                             key       = iv_key
                                                             value     = iv_value
                                                  IMPORTING  reg_value = rv_reg
                                                  EXCEPTIONS OTHERS    = 5 ).
    IF sy-subrc = 0.
      RETURN.
    ENDIF.
    RAISE EXCEPTION NEW cx_sy_itab_error( ). " any cx_dynamic_check exception will do
  ENDMETHOD.


  METHOD get_java_path.
    CONSTANTS c_java_base_key TYPE string VALUE 'SOFTWARE\JavaSoft\Java Runtime Environment' ##NO_TEXT.
...
CLASS lcl_progress_indicator IMPLEMENTATION.
  ...
  METHOD new.
    DATA(ls_cfg) = COND #( WHEN is_cfg IS SUPPLIED
                           THEN is_cfg
  ...
  METHOD constructor.
    mv_mode = get_mode( ).
    set_mode( COND #( WHEN iv_progress = abap_true
                      THEN c_enable
                      ELSE c_disable ) ).
  ENDMETHOD.
...
*                     'I' - ?? Integer?
    ...
    m_fill_att gs_cfg-progress            'Progress indicator'(c02)      'C'.
    m_fill_att gs_cfg-system_events       'System events'(c03)           'C'.
    m_fill_att gs_cfg-compact_trace       'Loops compacted'(c04)         'C'.
    m_fill_att gs_cfg-pattern             'Customer namespace'(c05)      'T'.

    m_fill_att sy-index                 'PlantUML Execution Mode'(c10) 'G'.   " Group
    m_fill_radio mv_mode_url            'PlantUML web service'(c11) 'MOD'.
    m_fill_radio mv_mode_txt            'Save text file'(c12)       'MOD'.
    m_fill_radio mv_mode_exe            'Local PlantUML '(c13)      'MOD'.

    m_fill_att ''                       'PlantUML Settings'(c20)       'G'.
    m_fill_att gs_cfg-scale             'Scale '(c21)                  'S'.
    m_fill_att gs_cfg-hpages            'H-Pages '(c22)                'S'.
    m_fill_att gs_cfg-vpages            'V-Pages '(c23)                'S'.

    m_fill_att gs_cfg-server_url        'PlantUML Server'(c25)         'S'.
    m_fill_att gs_cfg-local_path        'Local PlantUML path'(c26)     'S'.
    m_fill_att gs_cfg-java_jar          'Local PlantUML jar file'(c27) 'S'.
    m_fill_att gs_cfg-java_appl         'Local Java path'(c28)         'S'.  " Select-Options

    m_fill_att gs_cfg-handwritten       'Handwritten '(c30)           'C'.
    m_fill_att gs_cfg-shadowing         'Shadowing '(c31)             'C'.
    m_fill_att gs_cfg-display_source    'Display source '(c32)        'C'.
    m_fill_att gs_cfg-teoz_architecture 'Use TEOZ architecture '(c33) 'C'.
  ENDMETHOD.


  METHOD to_radiobutton.
    mv_mode_url = xsdbool( gs_cfg-output_mode = lcl_diagram_text=>c_mode_url ).
    mv_mode_exe = xsdbool( gs_cfg-output_mode = lcl_diagram_text=>c_mode_exe ).
    mv_mode_txt = xsdbool( gs_cfg-output_mode = lcl_diagram_text=>c_mode_txt ).
  ENDMETHOD.


  METHOD from_radiobutton.
    IF mv_mode_url = abap_true.
      gs_cfg-output_mode = lcl_diagram_text=>c_mode_url.
    ELSEIF mv_mode_exe = abap_true.
      gs_cfg-output_mode = lcl_diagram_text=>c_mode_exe.
    ELSEIF mv_mode_txt = abap_true.
  ...
  METHOD dialog.
    to_radiobutton( ).
    IF gs_cfg-skip_dialog = abap_true.
      RETURN.
    ENDIF.
    IF cl_ci_query_attributes=>generic( p_name       = CONV #( sy-repid )                    " unique screen ID
...
CLASS lcl_uml_mscgen IMPLEMENTATION.
  ...
  METHOD begin_loop.
    " TODO: parameter IV_FROM is never used (ABAP cleaner)
    " TODO: parameter IV_TIMES is only used in commented-out code (ABAP cleaner)
    " TODO: parameter IV_NAME is never used (ABAP cleaner)
...
CLASS lcl_logger IMPLEMENTATION.
  ...
  METHOD verify.
    cl_abap_unit_assert=>assert_equals( exp = it_exp
                                        act = mt_log
                                        msg = iv_msg ).
  ENDMETHOD.
...
CLASS lcl_abap_trace_logger DEFINITION
  INHERITING FROM lcl_abap_trace
...
CONSTANTS c_my_program TYPE program VALUE 'Y_TEST_PROG'.

CLASS ltc_bag DEFINITION CREATE PRIVATE FOR TESTING RISK LEVEL HARMLESS DURATION SHORT.
  PUBLIC SECTION.
...
CLASS lcl_satr_state IMPLEMENTATION.
  ...
  METHOD fill.
    ...
    m_fill iv_progindex it_traceprog.
    m_fill iv_methindex it_tracemeth.
  ENDMETHOD.
...
CLASS ltc_abap_trace DEFINITION
  ...
  PRIVATE SECTION.
    CONSTANTS c_textindex TYPE sytabix VALUE 1.
    CONSTANTS c_progindex TYPE sytabix VALUE 2. " > 1
    CONSTANTS c_methindex TYPE sytabix VALUE 3.
    CONSTANTS c_fm_index  TYPE sytabix VALUE 4.
...
CLASS ltc_abap_trace IMPLEMENTATION.
  ...
  METHOD set_constructor.
    parse( iv_id = c_id_class ).
    cl_abap_unit_assert=>assert_equals( exp = VALUE lcl_abap_trace=>ts_event( type   = c_type_class
  ...
  METHOD parse_method.
    parse( iv_id        = c_id_method
           iv_methindex = c_methindex ).
    mo_log->verify( it_exp = VALUE #( ( method = 'SET_METH'
                                        params = |{ c_progindex } { c_textindex } { c_methindex }| )
                                      ( method = 'MODULE'   params = |{ c_progindex }| )
  ...
  METHOD parse_function_module.
    ...
    mo_log->verify( it_exp = VALUE #( ( method = 'SET_FUNCTION_MODULE' params = |{ c_progindex } { c_textindex }| )
                                      ( method = 'EVENT'               params = |{ c_textindex }| )
  ...
  METHOD parse_submit.
    CONSTANTS c_mod_index TYPE sytabix VALUE 1. " larger 0, smaller than c_progindex
    CONSTANTS c_my_event  TYPE program VALUE 'MY_EVENT'.
...
CLASS ltc_sequence DEFINITION CREATE PRIVATE FOR TESTING RISK LEVEL HARMLESS DURATION SHORT.
  PUBLIC SECTION.
    INTERFACES lif_unit_test.

    TYPES tt_sat TYPE STANDARD TABLE OF ts_sat WITH KEY aus_tabix WITH NON-UNIQUE SORTED KEY k1 COMPONENTS aus_tabix. " Performance ?
...
*   add to collection
  ...
  METHOD sat_fixture_events.
    CONSTANTS c_prog  TYPE program VALUE 'Y_TEST_PROG'.
    CONSTANTS c_ctrl  TYPE program VALUE 'LCL_CTRL'.
    CONSTANTS c_model TYPE program VALUE 'LCL_MODEL'.

    rt_sat = VALUE tt_sat( id = 'm'
                           ( from_level = 1
                             caller     = VALUE #( global = c_prog
                                                   type   = c_type_prog )
                             called     = VALUE #( global = c_prog
                                                   type   = c_type_class
                           ...
                           ( from_level = 2
                             caller     = VALUE #( global = c_prog
                                                   type   = c_type_class
                                                   local  = c_ctrl mod = 'MAIN' )
                             called     = VALUE #( global   = 'SAPPROG'
                                                   type     = c_type_class
                                                   instance = 6
                                                   local    = c_model mod = lcl_abap_trace=>c_create_method ) )
                           ( from_level = 2
                             caller     = VALUE #( global = 'SAPPROG'
                                                   type   = c_type_class
  ...
  METHOD collect.
    cl_abap_unit_assert=>assert_equals( exp = VALUE tt_message(
  ...
  METHOD add.
    ...
    cl_abap_unit_assert=>assert_equals( exp = VALUE tt_message(
                                                        ...
                                                        ( from_level = 2
                                                          caller     = 2
                                                          called     = 3
                                                          msg        = lcl_abap_trace=>c_create_method )
                                                        ( from_level = 2  caller = 4  called = 2 msg = 'CHANGE'   ) )
                                        act = mo_sequence->mt_trace
...
CLASS ltc_class IMPLEMENTATION.
  ...
  METHOD section_name.
    cl_abap_unit_assert=>assert_equals( exp = '/IDV0/JN_CL_ASM_ACT==========='
                                        act = mo_class_name->to_internal( '/IDV0/JN_CL_ASM_ACT' )
                                        msg = 'LCL_CLASS SECTION_NAME( ) Translate Error' ).
  ENDMETHOD.


  METHOD technical_name.
    cl_abap_unit_assert=>assert_equals( exp = 'CL_GUI_ALV_GRID===============CP'
                                        act = mo_class_name->technical( 'CL_GUI_ALV_GRID' )
...
CLASS ltc_actors_factory DEFINITION
  CREATE PRIVATE
  FOR TESTING RISK LEVEL HARMLESS DURATION SHORT.
...
CLASS ltc_actors_factory IMPLEMENTATION.
  ...
  METHOD constructor.
    ms_exp = VALUE #( id         = 1
                      ...
                      caller     = VALUE #( global = c_my_program
                                            type   = c_type_fugr )
                      called     = VALUE #( global   = c_my_program
                                            local    = c_model
                                            type     = c_type_class
                                            instance = 1
  ...
  METHOD sat_fixture_messages.
    DATA(ls_main) = VALUE ts_call( global = c_prog
                                   type   = c_type_class
                                   local  = c_ctrl mod = 'MAIN' ).
  ...
  METHOD sat_fixture_plant_uml.
    rt_sat = VALUE ltc_sequence=>tt_sat(
                       ( id         = 1
                         ...
                         caller     = VALUE #( global = c_my_program
                                               type   = c_type_prog )
                         called     = VALUE #( type     = c_type_class
                                               instance = 0
                                               global   = c_my_program
                                               local    = 'LCL_CTRL' mod = 'MAIN' ) )
                       ( id         = 'm'
                         from_level = 2
                         caller     = VALUE #( type     = c_type_class
                                               instance = 0
                                               global   = c_my_program
                                               local    = 'LCL_CTRL' mod = 'MAIN' )
                         called     = VALUE #( type     = c_type_class
                                               instance = 1
                                               global   = c_my_program
                                               local    = 'LCL_MODEL' mod = lcl_abap_trace=>c_create_method ) )
                       ( id         = ' '
                         from_level = 3
                         caller     = VALUE #( type     = c_type_class
                                               instance = 447
                                               global   = c_my_program
                                               local    = 'LCL_CALL_STACK' mod = 'PLANTUML' )
                         called     = VALUE #( type     = c_type_class
                                               instance = 448
                                               global   = c_my_program
                                               local    = 'LCL_ITERATOR' mod = 'HAS_NEXT' ) )  ).
  ENDMETHOD.


  METHOD sat_fixture_record.
    rs_sat = VALUE ts_sat( id         = 'm'
                           from_level = 1
                           caller     = VALUE #( global = c_prog
                                                 type   = c_type_prog )
                           called     = VALUE #( global = c_prog
  ...
  METHOD put.
    ...
    cl_abap_unit_assert=>assert_equals( exp = lv_count
                                        act = get_count( )
...
CLASS ltc_actors IMPLEMENTATION.
  ...
  METHOD get_text_class.
    ...
    DATA(ls_msg) = VALUE ts_message( id          = iv_id
                                     from_level  = 1
                                     path-caller = 1
                                     path-called = 2
                                     msg         = 'CONSTRUCTOR' ).
    cl_abap_unit_assert=>assert_equals( exp = iv_exp
                                        act = mo_actors->lif_actors~short_text( ls_msg )
  ...
  METHOD call_constructor.
    DATA(ls_path) = mo_actors->lif_path~to_path( VALUE ts_sat( called = VALUE #( local = 'LCL_TEST'
                                                                                 type  = c_type_class ) ) ).
...
CLASS ltc_stack IMPLEMENTATION.
  ...
  METHOD pop.
    ...
    cl_abap_unit_assert=>assert_equals( exp = ms_exp
                                        act = mo_stack->pop( )
  ...
  METHOD push.
    mo_stack->push( ms_exp ).
    cl_abap_unit_assert=>assert_true( act = xsdbool(         ( ms_exp = mo_stack->pop( ) )
...
CLASS ltc_call IMPLEMENTATION.
  ...
  METHOD call.
    ...
    mo_uml->mo_log->verify( it_exp = VALUE lcl_logger=>tt_log( ( method = 'CALL'
...
CLASS ltc_iterator DEFINITION
  ...
  PRIVATE SECTION.
    ...
    METHODS has_previous_after_init  FOR TESTING.
    METHODS has_previous_at_first    FOR TESTING.
    METHODS has_previous_after_first FOR TESTING.
ENDCLASS.
...
CLASS ltc_iterator IMPLEMENTATION.
  ...
  METHOD constructor.
    ...
    mt_messages = VALUE #( ( id = 1 from_level = 1 caller = 1 called = c_main msg = 'MAIN' )  " PROG -> Main
  ...
  METHOD next.
    cl_abap_unit_assert=>assert_equals( exp = VALUE #( mo_iter->mt_list[ 1 ] )
                                        act = mo_iter->next( )
...
CLASS ltc_uml DEFINITION ABSTRACT
  CREATE PROTECTED
  FOR TESTING RISK LEVEL HARMLESS DURATION SHORT.
...
CLASS ltc_plant_uml IMPLEMENTATION.
  ...
  METHOD add.
    ...
    mo_uml->add( c_code ).
    cl_abap_unit_assert=>assert_equals( exp = lv_before && c_code
                                        act = mo_uml->mv_diagram
  ...
  METHOD plantuml_header.
    cl_abap_unit_assert=>assert_equals( exp = |scale 0.50\n|
                                        act = CAST lcl_uml_plant( mo_uml )->scale( lcl_configuration=>get( ) )
...
CLASS ltc_file_name DEFINITION
  ...
  PRIVATE SECTION.
    CONSTANTS c_path   TYPE string VALUE 'C:\Temp\Dokumente\UML\'.
    CONSTANTS c_prefix TYPE string VALUE 'test' ##NO_TEXT.
...
CLASS ltc_diagram IMPLEMENTATION.
  ...
  METHOD to_url.
    ...
    cl_abap_unit_assert=>assert_equals( exp = 'SyfFKj2rKt3CoKnELR1Io4ZDoSa70000'
...
CLASS ltc_messages DEFINITION
  ...
  PRIVATE SECTION.
    ...
    METHODS calls_for_empty_diagram    FOR TESTING.
    METHODS plantuml_for_empty_diagram FOR TESTING.
...
CLASS ltc_messages IMPLEMENTATION.
  ...
  METHOD to_uml.
    DATA(lo_calls) = ltc_sequence=>new_sequence( it_sat = it_sat
  ...
  METHOD calls_for_empty_diagram.
    ...
    get_log( )->verify( it_exp = VALUE lcl_logger=>tt_log( ( method = 'HEADER' )
                                                           ( method = 'FOOTER' ) )
...
|deactivate 2\n@enduml\n|.
  ...
  METHOD sample_plantuml_body.
    cl_abap_unit_assert=>assert_equals( exp = expected_plantuml_source( )
                                        act = to_uml( mt_sat )
  ...
  METHOD plantuml_for_empty_diagram.
    cl_abap_unit_assert=>assert_char_cp( exp = |@startuml\nhide footbox\nautonumber\n*skinparam*@enduml\n|
                                         act = to_uml( VALUE #( ) )
...
CLASS ltc_uml_compactor DEFINITION
  CREATE PRIVATE
  FOR TESTING RISK LEVEL HARMLESS DURATION SHORT.
...
*     Expected: Object of type LCL_MESSAGES created, not compatible with LCL_MESSAGE_COMPACT
  ...
  METHOD begin_loop.
    ...
    mo_iterator->begin( iv_idx  = mo_iterator->mv_idx
                        iv_from = 1 ).
    cl_abap_unit_assert=>assert_equals( exp = |loop 2 times\n|
                                        act = mo_uml->mv_diagram
  ...
  METHOD calls_for_begin_loop.
    ...
    mo_iterator->begin( iv_idx  = mo_iterator->mv_idx
                        iv_from = 1 ).
    CAST lcl_uml_logger( mo_uml )->mo_log->verify( it_exp = VALUE lcl_logger=>tt_log( ( method = 'BEGIN_LOOP'
                                                                                        params = '2' )  )
  ...
  METHOD end_loop.
    ...
    cl_abap_unit_assert=>assert_equals( exp = |end\n|
                                        act = mo_uml->mv_diagram
...
*       So this is just for developer information, for now
  ...
  METHOD clone_rest.
    ...
    fixture( 1 ).
    cl_abap_unit_assert=>assert_equals( exp = c_start - 1
                                        act = NEW lcl_pattern( it_xindex = mo_index->mt_xindex
  ...
  METHOD clone_resize.
    CONSTANTS c_start TYPE i VALUE 2.
    CONSTANTS c_size  TYPE i VALUE 3.

    fixture( 1 ).
    cl_abap_unit_assert=>assert_equals( exp = c_size
                                        act = NEW lcl_pattern( it_xindex = mo_index->mt_xindex
                                                               iv_start  = c_start
...
CLASS ltc_shrinkage IMPLEMENTATION.
  ...
  METHOD reconcile_loop_references.
    DATA(lt_cycles) = VALUE lcl_messages_compact=>tt_cycle( ( start = 1    end = 48   last = 192    times = 4 )    "   1 +  48 x 4 - 1 = 192
    ...
    cl_abap_unit_assert=>assert_equals( exp = VALUE lcl_messages_compact=>tt_cycle(
...
*   No filter / No conversion
  ...
  METHOD filter_message.
    mo_trace->mo_atra->it_traceprog   = VALUE #( ( cprog = 'Y_PROG'   object = c_type_prog  )
  ...
  METHOD fixture_prepare.
    mo_trace->mo_atra->it_traceprog = VALUE #( ( cprog = ltc_actors_factory=>c_prog  object = c_type_prog  )
                                               ( cprog = ltc_actors_factory=>c_prog  object = c_type_class  )
                                               ( cprog = 'SAPPROG'  object = c_type_class )  ).
    mo_trace->mo_atra->it_tracetext = VALUE #( ( tracetext = 'MAIN' )
    ...
    mo_trace->mo_atra->it_tracemeth = VALUE #( ( methode = 'MAIN'  )
...
*   Fixture: Scenario from ltc_objects=>sat_fixture_events( )
    ...
    mo_trace->mo_atra->it_austab_hier = VALUE #(
        event = '>'
        ( id = 'O'             index = 1  ebene = 0  caller = 0 )
        ( id = 'C' subid = 'R' index = 2  ebene = 1  caller = 1 textindex = 6 )  " Submit
        ( id = 'V' subid = 'O' index = 3  ebene = 2  caller = 2 )  " DB Op.
        ( id = 'P'             index = 4  ebene = 2  caller = 2 progindex = 1 textindex = 1 )  " Program

        ( id = 'm'             index = 5  ebene = 3  caller = 4 progindex = 2 textindex = 4 methindex = 1 )
        ( id = 'm' subid = 'x' index = 6  ebene = 4  caller = 5 progindex = 3 textindex = 5 methindex = 2 )
        ( id = 'm' subid = 'x' index = 7  ebene = 4  caller = 6 progindex = 1 textindex = 4 methindex = 3 ) ).
    fill_trace( ).
  ENDMETHOD.
  ...
  METHOD add_more.
    ...
    DATA(lt_exp) = VALUE tt_message( ( id         = 'C'
                                       from_level = 1
                                       caller     = 1
                                       called     = 2
                                       msg        = ltc_actors_factory=>c_prog )
                                     ( id         = 'm'
                                       from_level = 3
                                       caller     = 2
                                       called     = 3
...
*   TO DO: check coverage/review fixture
    cl_abap_unit_assert=>assert_equals(
        exp = VALUE lcl_trace=>tt_seq_k1( ( aus_tabix       = 1 )
                                          ( called    = VALUE #( global = ltc_actors_factory=>c_prog
                                                                 type   = c_type_prog
...
CLASS ltc_filter_custom IMPLEMENTATION.
  ...
  METHOD sample_sat.
    rs_sat = VALUE ts_sat( id         = 'L'
                           from_level = 8
                           aus_tabix  = 21
                           caller     = VALUE #( global = 'SAPMSSYD'
                                                 type   = 'PROG' mod = '%_IMODE_INIT' )
                           called     = VALUE #( ) ).
  ENDMETHOD.
...
CLASS ltc_configuration IMPLEMENTATION.
  ...
  METHOD from_radiobutton_txt.
    ...
    cl_abap_unit_assert=>assert_equals( exp = lcl_diagram_text=>c_mode_txt
                                        act = mo_config->gs_cfg-output_mode
